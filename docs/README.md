...
> **Status:** Living documentation set · **Last updated:** 2026-06-21 · **Audience:** build agents & maintainers
> **Purpose:** Single entry point and source of truth for the project. Read this first, then jump to the doc for your task.

> ### 🔄 Current direction (2026-06-21): foundation-first **beat-grid studio**
> The build was reset to ground the foundation before piling on features: blind ADT on dense/distorted metal is unreliable, and **everything downstream snaps to the beat grid**, so the grid must be accurate, drift-tracking, and *correctable* first. **`main` now hosts a beat-grid & song-structure studio** (Clone-Hero highway with a metronome click + a DAW timeline with waveform/beats/bars/sections/tempo-drift curve). Run `python -m charter.studio mp3/clay.mp3`. The full auto-charter (ADT engines, pattern mode) is preserved on branch **`studio-autocharter-v1`**. **See [HANDOFF.md](./HANDOFF.md) for the live state.** The numbered design docs below remain the format/pipeline source of truth and are still valid.
...
1. **Current state (2026-06-21):** **Reset to a foundation-first beat-grid studio on `main`** (run `python -m charter.studio mp3/clay.mp3`): a Clone-Hero highway (metronome click) + a DAW timeline (waveform / beats / bars / colored sections / tempo-drift curve) to detect and **correct** the beat grid + song structure before building on it. The symbolic backend (`drumnote`, `mapping`) and audio pipeline (`audio`) are intact and reused; scan-chart gate still applies. The earlier "No Part" issue was a **CH controller setting** (set the controller to drums — resolved). The full auto-charter (baseline + **DrumSep per-drum engine**, genre **pattern mode** + kick-from-audio hybrid, metal accuracy controls) is **preserved on branch `studio-autocharter-v1`** — valid work, paused. See [HANDOFF.md](./HANDOFF.md) for the live state and next steps.
