...
1. **Current state (2026-06-20):** The pipeline runs **end-to-end** — `charter/` turns an **audio file into a playable song folder** (`notes.chart` + `song.ini` + `song.opus`) that Clone Hero's own parser (scan-chart) detects as `fourLanePro`, `playable=True`; **37 tests pass.** Roadmap **Phases 1–3 + the Phase-4 gate are built** on a dependency-light baseline (numpy/scipy + FFmpeg). It is **baseline-grade**: kick/snare/hi-hat backbone only (no toms / ride-vs-crash), tested on synthetic drums — quality on real music is unmeasured. The SOTA tools (Demucs/Beat This!/ADTOF) are wired as **optional adapters** used if installed; treat them and their RISK tags per [08](./08-tech-stack-and-deployment.md).
