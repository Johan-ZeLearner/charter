...
1. **Current state (2026-06-20):** The **symbolic back-half is built** (roadmap Phases 1–2) — `charter/` turns GM-drum MIDI into a `.chart` that Clone Hero's own parser (scan-chart) detects as `fourLanePro`; 23 tests pass. The **audio front-half (Stages 0–5) is not started.** See the project `README.md` for run/test commands. Everything in the audio half is still design only — treat those tools as *candidates* with maturity caveats; several are flagged **not turnkey** (see RISK tags and [08](./08-tech-stack-and-deployment.md)).
2. **Where to start building:** [10 MVP Roadmap](./10-mvp-roadmap.md) — Phases 1–2 are done; **Phase 3 (audio frontend with easy fallbacks: Demucs → ADTOF → Beat This!) is next.** The organizing principle is **decoupling** — the symbolic back-half (Stages 6–8: `events → DrumNote → .chart → scan-chart`) was built **first**, on hand-made GM MIDI with **zero ML**, so a format bug can never be confused with a transcription bug. The next milestone is **a green scan-chart on a real song** (an actual mp3 through the full pipeline).
