...
1. **Current state:** This is a **research-grounded design**, not yet code. No pipeline exists yet — these docs are the plan. Treat every "tool" as a *candidate* with the maturity caveat attached; several key repos are flagged **not turnkey** (see RISK tags and [08](./08-tech-stack-and-deployment.md)).
2. **Where to start building:** [10 MVP Roadmap](./10-mvp-roadmap.md). The organizing principle is **decoupling** — build the symbolic back-half (Stages 6–8: `events → DrumNote → .chart → scan-chart`) **first**, on hand-made GM MIDI with **zero ML**, so a format bug can never be confused with a transcription bug. The first real milestone is **a green scan-chart on a real song**.
